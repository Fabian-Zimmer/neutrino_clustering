...
for m_Cl in nu_m_picks:
    ...
    Cls_z0p15_raw = jnp.load(f"{Cl_folder}/Cls_z=0p15_m={m_Cl}eV.npy")
    Cls_z0_raw = jnp.load(f"{Cl_folder}/Cls_z=0_m={m_Cl}eV.npy")
    # (q momentum bins, l multipoles) = (50, 20)

    # Add monopole
    Cl_z0p15 = jnp.insert(Cls_z0p15_raw, 0, 0, axis=1)
    ...
    for qi in range(Primordial.n_qbins):
        ...
        np.random.seed(5)
        Tmap_z0p15 = hp.sphtfunc.synfast(
            Cl_z0p15[qi], nside=simdata.Nside, lmax=None, pol=False)
        np.random.seed(5)  #note: seed needs to reset
        Tmap_z0 = hp.sphtfunc.synfast(
            Cl_z0[qi], nside=simdata.Nside, lmax=None, pol=False)

        Deltas_z0p15 = Tmap_z0p15 * Primordial.uK / (Params.T_CNB*(1+0.15))
        Deltas_z0 = Tmap_z0 * Primordial.uK / (Params.T_CNB)
        # (Npix)

        Deltas_z0p15_q_l.append(Deltas_z0p15)
        Deltas_z0_q_l.append(Deltas_z0)

    Deltas_z0p15_m_l.append(jnp.array(Deltas_z0p15_q_l))
    Deltas_z0_m_l.append(jnp.array(Deltas_z0_q_l))


Deltas_z0p15_matrix = jnp.array(Deltas_z0p15_m_l)
Deltas_z0_matrix = jnp.array(Deltas_z0_m_l)
# (masses, q momentum bins, Npix)
jnp.save(f"{Delta_folder}/Delta_matrix_z0p15.npy", Deltas_z0p15_matrix)
...
for halo_j in range(int(pars.halo_num)):
    ...
    # Momenta at z=0 and z=0.15
    p_z0 = p_arr[..., 0]
    p_z0p15 = p_arr[...,-1]
    # (masses, Npix, neutrinos per pixel)

    # Cl momenta are expressed in terms of T_CNB(z=0), but for proper matching 
    # we need momenta at z=0.15 in terms of T_CNB(z=0.15).
    q_z0p15 = y_arr[...,-1]/(1+0.15)
    # (masses, Npix, neutrinos per pixel)

    # Find indices to match neutrino momenta to Cl momenta
    q_idx = jnp.abs(
        Primordial.Cl_qs[None,None,None,:] - q_z0p15[...,None]).argmin(axis=-1)
    ...
    Deltas_halo = jnp.reshape(
        Deltas_z0p15_matrix[m_idx, q_idx, p_idx], 
        (m_num, simdata.Npix, simdata.p_num))
    Deltas_halos_l.append(Deltas_halo)
    #! check if Deltas_z0p15_matrix is used (did some manual tests with z0)
    # (masses, Npix, neutrinos per pixel)
    ...
    pix_dens_halo = Physics.number_density_Delta(
        p_z0, p_z0p15, Deltas_halo, simdata.pix_sr, Params())
    ...
    tot_dens_halo = Physics.number_density_Delta(
        ...
        p_z0p15.reshape(m_num, -1), 
        Deltas_halo.reshape(m_num, -1), 
        4*Params.Pi, Params())
    tot_dens_incl_PFs_l.append(tot_dens_halo)


#! check if file names end in z0p15 (did some manual tests with z0)
jnp.save(
