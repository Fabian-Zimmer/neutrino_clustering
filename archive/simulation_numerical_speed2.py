...
# Create an interpolator object for z
# z_interp = interp1d(s_int_steps, z_int_steps, assume_sorted=True)
...
def EOMs(s_val, y):
    ...
    if np.all(np.abs(x_i) < snap_GRID_L):
        ...
        dPsi_grid = data_files[fname]['dPsi_grid']
        cell_grid = data_files[fname]['cell_grid']
        cell_gens = data_files[fname]['cell_gens']
...
def backtrack_1_neutrino(y0_Nr):
    ...
    np.save(
        f'{temp_dir}/nu_{int(Nr)}.npy',  
        np.array(sol.y.T, dtype=np.float32)
    )
...
for halo_j, halo_ID in enumerate(halo_batch_IDs):
    ...
    # Pre-load data outside the EOMs function
    data_files = {}
