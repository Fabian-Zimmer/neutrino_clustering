...
def nu_in_which_cell(x_i, cell_coords, cell_gens, init_GRID_S):
    ...
    for i in range(num_cells):
        cell_center = cell_coords[i]
        cell_len = cell_lens[i]
        
        # Check if the neutrino is within the cell boundaries
        in_cell = (
            (np.abs(x_i[0] - cell_center[0]) < cell_len) &
            (np.abs(x_i[1] - cell_center[1]) < cell_len) &
            (np.abs(x_i[2] - cell_center[2]) < cell_len)
        )
...
def EOMs(
    s_val, y, 
    snap_data_dPsi_grid, snap_data_cell_grid, snap_data_cell_gens
):
    # Initialize vector.
    ...
    if np.all(np.abs(x_i) < snap_GRID_L):
        ...
        dPsi_grid = snap_data_dPsi_grid[idx]
        cell_grid = snap_data_cell_grid[idx]
        cell_gens = snap_data_cell_gens[idx]
    ...
    dyds = -np.concatenate(
        (
        np.expand_dims(u_i, axis=0), 
        np.expand_dims(1./(1.+z)**2 * grad_tot, axis=0)
        ), 
        axis=0
    )
...
def backtrack_1_neutrino(y0_Nr):
    ...
    sol = solve_ivp(
        fun=EOMs, 
        t_span=[s_int_steps[0], s_int_steps[-1]], 
        t_eval=s_int_steps,
        y0=y0, 
        method=integration_solver, 
        vectorized=True,
        args=(
            snap_data_dPsi_grid, 
            snap_data_cell_grid, 
...
for halo_j, halo_ID in enumerate(halo_batch_IDs):
    ...
    snap_data_dPsi_grid = nb.typed.List()
    snap_data_cell_grid = nb.typed.List()
    snap_data_cell_gens = nb.typed.List()

    for snap in nums_snaps:
        fname = f'origID{halo_ID}_snap_{snap}'
        snap_data_dPsi_grid.append(
            np.load(f'{temp_dir}/dPsi_grid_{fname}.npy').copy()
        )
        snap_data_cell_grid.append(
            np.squeeze(np.load(f'{temp_dir}/fin_grid_{fname}.npy')).copy()
        )
        snap_data_cell_gens.append(
            np.load(f'{temp_dir}/cell_gen_{fname}.npy').copy()
        )


    def check_incompatible_values(array_list):
        for idx, arr in enumerate(array_list):
            if not np.isfinite(arr).all():
                print(f"Array {idx} contains incompatible values:")
    ...
    # Test the function
    if check_incompatible_values(snap_data_dPsi_grid):
        print("snap_data_dPsi_grid is fine")
    else:
        print("snap_data_dPsi_grid contains incompatible values")

    if check_incompatible_values(snap_data_cell_grid):
        print("snap_data_cell_grid is fine")
    else:
        print("snap_data_cell_grid contains incompatible values")

    if check_incompatible_values(snap_data_cell_gens):
        print("snap_data_cell_gens is fine")
    else:
        print("snap_data_cell_gens contains incompatible values")
    ...
    if args.sim_type in ('single_halos', 'spheres', 'benchmark'):
        ...
        for cp, ui_elem in enumerate(ui):
            ...
            backtrack_1_neutrino(y0_Nr[131])
            backtrack_1_neutrino(y0_Nr[132])
            backtrack_1_neutrino(y0_Nr[133])

            # Run simulation on multiple cores.
            # with ProcessPoolExecutor(CPUs_sim) as ex:
            #     ex.map(backtrack_1_neutrino, y0_Nr[100:150])
