...
def check_grid(init_grid, DM_pos, parent_GRID_S, DM_lim):
    ...
    cell_len = parent_GRID_S / 2.

    # Select DM particles inside each cell based on cube length generation.
    DM_in_cell_IDs = np.asarray(
        (np.abs(DM_pos[...,0] - init_grid[...,0]) < cell_len) & 
        (np.abs(DM_pos[...,1] - init_grid[...,1]) < cell_len) & 
        (np.abs(DM_pos[...,2] - init_grid[...,2]) < cell_len)
    )
    ...
    DM_count_sync = DM_count_stable[:, np.newaxis]
    ...
    cell_com += np.squeeze(np.delete(init_grid, ~stable_cells, axis=0), axis=1)
...
def cell_division(
    ...
    DM_count_l, cell_com_l, cell_gen_l = [], [], []
    thresh = 1
    ...
    while thresh > 0:
        ...
        if thresh == 0:
            ...
            cell_gen_l.append(
                np.full(len(init_grid), cell_division_count, dtype=int)
            )
            ...
            cell_gen_l.append(
                np.full(len(no_parent_cells), cell_division_count, dtype=int)
            )
...
def cell_gravity_short_range(
    ...
    cell_len = init_GRID_S / (2 ** (np.array(cell_gen) + 1))

    # Center all DM positions w.r.t. cell center.
    # DM_pos already in shape = (1, DM_particles, 3)
    DM_pos_cent = DM_pos - np.array(cell_coords_in)[:, np.newaxis, :]
...
def cell_gravity_long_range_quadrupole(
    ...
    cellC_sync = np.repeat(cellC_cc[np.newaxis,...], len(cell_com), axis=0) 
    cellC_sync -= cell_com
    ...
    cellC_dis = np.linalg.norm(cellC_sync, axis=1)
    ...
    DMj_dis = np.linalg.norm(DM_mpoles, axis=2)[...,np.newaxis]

    # Array, where cell C is centered on c.o.m. coords. of J cells.
    cellC_Jcoms = np.repeat(
        np.expand_dims(cellC_cc, axis=(0,1)), len(mpoles_com), axis=0
    )
    cellC_Jcoms -= mpoles_com

    # Distance of cell C to c.o.m. of multipole cells.
    cellC_Jdis = np.linalg.norm(cellC_Jcoms, axis=2)
    ...
    cellC_Jcoms_mono = np.repeat(
        np.expand_dims(cellC_cc, axis=(0,1)), len(mono_com), axis=0
    )
    cellC_Jcoms_mono -= mono_com

    # Distance of cell C to c.o.m. of monopole cells.
    cellC_dis_mono = np.linalg.norm(cellC_Jcoms_mono, axis=2)
    ...
    dPsi_monopole_cells = G*DM_sim_mass*np.nansum(
        DM_count[:,np.newaxis]*cellC_Jcoms_mono_2D/(cellC_dis_mono**3), axis=0)
