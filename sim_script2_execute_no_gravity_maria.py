...
angle_momentum_decay = jnp.load(
    f'sim_output/no_gravity/neutrino_angle_momentum_decay.npy') 
decayed_neutrinos_z = jnp.load(
    f'sim_output/no_gravity/decayed_neutrinos_z.npy')
z_array = jnp.load(
...
def backtrack_1_neutrino(init_vector, s_int_steps,angle_momentum_decay,decayed_neutrinos_z,z_array,neutrino_momenta):
    ...
    sol = diffrax.diffeqsolve(
        ...
        saveat=saveat, stepsize_controller=stepsize_controller, args=(int(Nr),angle_momentum_decay,decayed_neutrinos_z,z_array, neutrino_momenta))
...
@jax.jit
def simulate_neutrinos_1_pix(init_xyz, init_vels, s_int_steps, angle_momentum_decay,decayed_neutrinos_z,z_array,neutrino_momenta):
    """
...
with ProcessPoolExecutor(CPUs_sim) as executor:
    futures = [
        executor.submit(
            simulate_neutrinos_1_pix, init_xyz, init_vels[pixel], s_int_steps, angle_momentum_decay,decayed_neutrinos_z,z_array,neutrino_momenta) for pixel in range(Npix)]
