...
def main_radius(DM_radius):
    ...
    for j, (snap, proj) in enumerate(zip(NUMS_SNAPSHOTS[::-1], prog_idx)):
        ...
        np.save(f'CubeSpace/dPsi_grid_snapshot_{snap}_{m0}Msun_{DM_radius}kpc.npy', dPsi_combined)

        # Delete temp. files.
        fct.delete_temp_data('CubeSpace/DM_positions_*')
        fct.delete_temp_data('CubeSpace/dPsi_*batch*.npy') 

        print(f'snapshot {snap} : cell division rounds: {cell_division_count}')

    np.save(f'CubeSpace/NrDM_snapshot_{snap}_{m0}Msun_{DM_radius}kpc.npy', NrDM_snaps[::-1])
