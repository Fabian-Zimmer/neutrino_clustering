...
for halo_j, halo_ID in enumerate(halo_batch_IDs):
    ...
    for c, init_xyz in enumerate(reduced_ccs_kpc):
        ...
        jnp.save(f'{pars.directory}/vectors_{end_str}_cell{c+1}.npy', nu_vectors[...,-1,:])  # only save vectors at z_sim boundary
        ...
        pix_dens = Physics.number_densities_all_sky(
            ...
            pix_sr=pix_sr_sim,
            args=Params())
        pix_dens_cell_l.append(jnp.squeeze(pix_dens))
        ...
        tot_dens = Physics.number_densities_mass_range(
            ...
            pix_sr=4*Params.Pi,
            args=Params())
        tot_dens_cell_l.append(jnp.squeeze(tot_dens))
        
    # Save all pixel/total number densities for all cells for current halo
    pix_dens_halo_l.append(pix_dens_cell_l)
    tot_dens_halo_l.append(tot_dens_cell_l)
...
jnp.save(f"{pars.directory}/pixel_densities.npy", jnp.array(pix_dens_halo_l))
jnp.save(f"{pars.directory}/total_densities.npy", jnp.array(tot_dens_halo_l))
