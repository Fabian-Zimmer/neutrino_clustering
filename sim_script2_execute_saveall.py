...
for halo_j, halo_ID in enumerate(halo_batch_IDs):
    ...
    def compute_density_zi(zi, nu_vectors, nu_massrange):
        select = [0,zi]
        z_dens = Physics.number_densities_mass_range(
            v_arr=nu_vectors.reshape(-1, 100, 6)[..., select, 3:], 
            m_arr=nu_massrange, 
            pix_sr=4*Params.Pi,
            args=Params())
        return z_dens
    ...
    compute_density_zi(1, nu_vectors, nu_massrange)

    # Parallel computation
    # with ProcessPoolExecutor(50) as executor:
    #     futures = [
    #         executor.submit(
    #             compute_density_zi, zi, nu_vectors, nu_massrange) 
    #             for zi in jnp.arange(1,99,50)
    #     ]

    #     # Collect results
    #     z_densities_1_halo_l = [future.result() for future in futures]
    # zed_dens_l.append(z_densities_1_halo_l)
...
# jnp.save(f"{pars.directory}/zeds_densities.npy", jnp.array(zed_dens_l))
