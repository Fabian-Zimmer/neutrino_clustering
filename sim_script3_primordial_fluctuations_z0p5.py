...
for m_Cl in nu_m_picks:
    ...
    Cls_z0p5_raw = jnp.load(f"{Cl_folder}/Cls_z=0p5_m={m_Cl}eV.npy")
    Cls_z0_raw = jnp.load(f"{Cl_folder}/Cls_z=0_m={m_Cl}eV.npy")
    # (q momentum bins, l multipoles) = (50, 20)

    # Add monopole
    Cl_z0p5 = jnp.insert(Cls_z0p5_raw, 0, 0, axis=1)
    ...
    for qi in range(Primordial.n_qbins):
        ...
        np.random.seed(5)
        Tmap_z0p5 = hp.sphtfunc.synfast(
            Cl_z0p5[qi], nside=simdata.Nside, lmax=None, pol=False)
        np.random.seed(5)  #note: seed needs to reset
        Tmap_z0 = hp.sphtfunc.synfast(
            Cl_z0[qi], nside=simdata.Nside, lmax=None, pol=False)

        Deltas_z0p5 = Tmap_z0p5 * Primordial.uK / (Params.T_CNB*(1+0.5))
        Deltas_z0 = Tmap_z0 * Primordial.uK / (Params.T_CNB)
        # (Npix)

        Deltas_z0p5_q_l.append(Deltas_z0p5)
        Deltas_z0_q_l.append(Deltas_z0)

    Deltas_z0p5_m_l.append(jnp.array(Deltas_z0p5_q_l))
    Deltas_z0_m_l.append(jnp.array(Deltas_z0_q_l))


Deltas_z0p5_matrix = jnp.array(Deltas_z0p5_m_l)
Deltas_z0_matrix = jnp.array(Deltas_z0_m_l)
# (masses, q momentum bins, Npix)
jnp.save(f"{Delta_folder}/Delta_matrix_z0p5.npy", Deltas_z0p5_matrix)
...
for halo_j in range(int(pars.halo_num)):
    ...
    Deltas_halo = jnp.reshape(
        Deltas_z0p5_matrix[m_idx, q_idx, p_idx], 
        (m_num, simdata.Npix, simdata.p_num))
    Deltas_halos_l.append(Deltas_halo)
    #! check if Deltas_z0p5_matrix is used (did some manual tests with z0)
    # (masses, Npix, neutrinos per pixel)
    ...
    pix_dens_halo = Physics.number_density_Delta(
        p_z0, p_z0p5, Deltas_halo, simdata.pix_sr, Params())
    ...
    tot_dens_halo = Physics.number_density_Delta(
        ...
        p_z0p5.reshape(m_num, -1), 
        Deltas_halo.reshape(m_num, -1), 
        4*Params.Pi, Params())
    tot_dens_incl_PFs_l.append(tot_dens_halo)


#! check if file names end in z0p5 (did some manual tests with z0)
jnp.save(
