...
#! file paths are hardcoded, since script is only used once
sim_dir = f"sim_output/benchmark"
...
FCT_H0 = FCT_h*100*Params.km/Params.s/Params.Mpc
...
def halo_sample_z(z, snap, Mvir_z0, DM_mass, out_dir, args, origin_offset=0.):
    ...
    Mz = commah_output['Mz'][0,0]*args.Msun
    ...
    R_200 = np.power(Mz / (200*rho_crit(z, args)*4/3*args.Pi), 1./3.)
    R_s = R_200 / c_200
    ...
    invf = inversefunc(Proj, args=(R_s/args.kpc, f_200))  
    ...
    phis = np.random.uniform(0, 2*args.Pi, num_DM)  # uniform [0,2pi)
...
# note: from another sim_output folder
sim_dir_files = f"sim_output/Dopri5_1k"
halo_params = np.load(glob.glob(f'{sim_dir_files}/halo*params.npy')[0])
...
#         repeat(Mvir_med), repeat(DM_mass), repeat(benchmark_outdir), repeat(Params())
#     )
...
print(np.log10(Params.Mvir_MW/Params.Msun), np.log10(DM_mass*benchmark_DM[-1]/Params.Msun))

nums_proxy = np.arange(12, 36+1)
plt.plot(nums_proxy, benchmark_DM)
plt.savefig(f"{sim_dir}/mass_check.pdf")